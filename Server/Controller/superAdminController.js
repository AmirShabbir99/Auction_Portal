...
export const getAllPaymentProofs = catchAsyncErrors(async (req, res, next) => {
  let paymentProofs = await Payment.find();
  res.status(200).json({
    success: true,
    paymentProofs,
  });
...
export const getPaymentProofDetail = catchAsyncErrors(
  async (req, res, next) => {
    const { id } = req.params;
    const paymentProofDetail = await Payment.findById(id);
    res.status(200).json({
      success: true,
      paymentProofDetail,
    });
...
export const updateProofStatus = catchAsyncErrors(async (req, res, next) => {
  ...
  let proof = await Payment.findById(id);
  if (!proof) {
    return next(new ErrorHandler("Payment proof not found.", 404));
  }
  proof = await Payment.findByIdAndUpdate(
    id,
    { status, amount },
    {
      new: true,
...
export const deletePaymentProof = catchAsyncErrors(async (req, res, next) => {
  const { id } = req.params;
  const proof = await Payment.findById(id);
  if (!proof) {
    return next(new ErrorHandler("Payment proof not found.", 404));
  }
